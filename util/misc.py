...
def broadcast_xla_master_model_param(model, args):
    ...
    """
    import time
    save_path = os.path.join(args.output_dir, "checkpoint-init.pth")
    # save the state dict from the master process
    state_dict = model.state_dict()
    xm.save(state_dict, save_path, global_master=True)
    # wait to compensate for any latency in the NFS file system across nodes
    time.sleep(5)
    # load the state dict from disk in all processes
